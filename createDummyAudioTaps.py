...
for user in users:
    for day, switch in enumerate(user["days"]):
        if switch:
            ...
            while num_timestamps < 6:
                timestamp = generate_timestamps(start_time, 1, 0, 0)[0]
                taps.append(
                    {
                        "dittiId": user["user_permission_id"],
                        "audioFileTitle": "Creek",
                        "time": f"new Date(\"{timestamp.isoformat()}\")",
                        "timezone": "GMT-04:00 Eastern Daylight Time",
                        "action": "play" if num_timestamps == 0 else "restart",
                    }
                )
                start_time = start_time + datetime.timedelta(minutes=5, seconds=random.randint(3, 60))
                num_timestamps += 1

with open("dummyAuidoTaps.json", "w") as f:
