...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "log_type",
            sa.Enum("auto_detected", "manual", name="sleeplogtypeenum"),
            nullable=False,
        ),
        ...
        sa.Column(
            "type",
            sa.Enum("stages", "classic", name="sleepcategorytypeenum"),
            nullable=False,
        ),
    ...
    with op.batch_alter_table("sleep_log", schema=None) as batch_op:
        batch_op.create_index(
            batch_op.f("ix_sleep_log_date_of_sleep"),
            ["date_of_sleep"],
            unique=False,
        )
        batch_op.create_index(
            batch_op.f("ix_sleep_log_log_id"), ["log_id"], unique=True
        )
        batch_op.create_index(
            batch_op.f("ix_sleep_log_study_subject_id"),
            ["study_subject_id"],
            unique=False,
        )
    ...
    with op.batch_alter_table("sleep_level", schema=None) as batch_op:
        ...
        batch_op.create_index(
            batch_op.f("ix_sleep_level_date_time"), ["date_time"], unique=False
        )
    ...
    op.execute(
        "UPDATE study SET default_expiry_delta = 30 "
        "WHERE default_expiry_delta IS NULL"
    )
    op.execute("UPDATE study SET is_qi = false WHERE is_qi IS NULL")
    ...
    with op.batch_alter_table(
        ...
        batch_op.alter_column(
            "expires_on", existing_type=postgresql.TIMESTAMP(), nullable=True
        )
    ...
    # Note: Relationships are handled in ORM models and do not require
    # migration scripts.
    ...
    # Note: Event listeners are part of the application code and are
    # not included in migration scripts.
...
def downgrade():
    ...
    # Note: Event listeners are part of the application code and
    # are not included in migration scripts.
    ...
    with op.batch_alter_table(
        ...
        batch_op.alter_column(
            "expires_on", existing_type=postgresql.TIMESTAMP(), nullable=False
        )
    ...
    with op.batch_alter_table("study", schema=None) as batch_op:
        ...
        batch_op.alter_column(
            "default_expiry_delta", existing_type=sa.INTEGER(), nullable=True
        )
    ...
    sleeplogtypeenum = postgresql.ENUM(
        "auto_detected", "manual", name="sleeplogtypeenum"
    )
    sleepcategorytypeenum = postgresql.ENUM(
        "stages", "classic", name="sleepcategorytypeenum"
    )
