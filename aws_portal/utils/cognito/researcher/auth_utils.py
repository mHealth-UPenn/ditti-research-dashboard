...
def parse_and_validate_id_token(id_token, required_nonce=None):
    ...
    try:
        ...
        userinfo = oauth.researcher_oidc.parse_id_token(
...
def validate_token_for_authenticated_route(id_token):
    ...
    try:
        ...
        # Check issuer
        expected_issuer = f"https://cognito-idp.{current_app.config['COGNITO_RESEARCHER_REGION']}.amazonaws.com/{current_app.config['COGNITO_RESEARCHER_USER_POOL_ID']}"
        if unverified_claims.get("iss") != expected_issuer:
            logger.error(f"Invalid issuer: {unverified_claims.get('iss')}")
            return False, "Invalid token issuer"

        # Check audience
        expected_audience = current_app.config["COGNITO_RESEARCHER_CLIENT_ID"]
        ...
        try:
            ...
            jwks_url = f"https://cognito-idp.{current_app.config['COGNITO_RESEARCHER_REGION']}.amazonaws.com/{current_app.config['COGNITO_RESEARCHER_USER_POOL_ID']}/.well-known/jwks.json"
...
def get_account_from_email(email):
    """
    Find an account in the database by email.

    Args:
        email (str): The researcher's email address
    ...
    try:
        ...
        if account.is_archived:
            ...
            return None, "Account is archived"

        return account, None

    except Exception as e:
        logger.error(f"Database error retrieving account: {str(e)}")
