...
def create_studies(app):
    ...
    """
    study1 = Study(name="Study 1", acronym="STUD1",
                   ditti_id="D001", email="study1@example.com")
    study2 = Study(name="Study 2", acronym="STUD2",
                   ditti_id="D002", email="study2@example.com")
...
def get_api_entry(api_id, api_user_uuid, scope):
    ...
    Removed access_key_uuid and refresh_key_uuid as they are no longer stored in StudySubject.
    """
    return {
        "id": api_id,
...
def create_study_subject(post_admin, create_studies, create_apis, app):
    ...
    def _create(email, studies=None, apis=None):
        ...
        subject = StudySubject.query.filter(
            StudySubject.email == email).first()
...
def edit_study_subject(post_admin, subject_id, edit_payload):
    ...
    """
    try:
        res_edit = post_admin("/admin/study_subject/edit",
...
def test_study_subject_create(post_admin, create_studies, create_apis):
    ...
    subject = StudySubject.query.filter(
        StudySubject.email == "study_subject_create@example.com").first()
    assert subject is not None
...
def test_study_subject_archive(post_admin, create_study_subject, create_studies, create_apis):
    ...
    res_create = post_admin("/admin/study_subject/create",
    ...
    subject = StudySubject.query.filter(
        StudySubject.email == "study_subject_archive@example.com").first()
    assert subject is not None
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            [
                {"study_id": 1, "did_consent": False, "expires_on": datetime(
                    2025, 12, 31, 23, 59, 59, tzinfo=timezone.utc)},
                {"study_id": 2, "did_consent": True, "expires_on": datetime(
        ...
        (
            ...
            {"apis": [
                get_api_entry(1, "updated-api-user-uuid", ["read", "write"]),
                get_api_entry(2, "new-api-user-uuid", ["read"])
            ]},
            None,  # No change to email
            None,  # No change to studies
            [
                {"api_id": 1, "api_user_uuid": "updated-api-user-uuid",
                    "scope": ["read", "write"]},
                {"api_id": 2, "api_user_uuid": "new-api-user-uuid",
                    "scope": ["read"]}
            ]
        ),
        (
    ...
    if expected_studies is not None:
        ...
        for study, expected in zip(edited_subject.studies, expected_studies):
            ...
            assert study.expires_on.replace(
                tzinfo=timezone.utc) == expected["expires_on"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            # Assuming "existing_email1@example.com" already exists
...
def test_study_subject_edit_errors(
    ...
    if test_name == "Associate Archived Study":
        ...
        archived_study = Study(name="Archived Study", acronym="ARCH",
                               ditti_id="ARCH001", email="archived_study@example.com")
