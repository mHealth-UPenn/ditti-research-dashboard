...
def participant_auth_client(client, study_subject):
    ...
    with patch(
        "backend.auth.controllers.ParticipantAuthController.get_user_from_token"
    ) as mock_get_user:
...
def test_fitbit_authorize_success(
    app, participant_auth_client, study_subject, fitbit_api
):
    # Mock generate_code_verifier and create_code_challenge
    with (
        patch(
            "backend.views.api.fitbit.generate_code_verifier"
        ) as mock_gen_verifier,
        patch(
            "backend.views.api.fitbit.create_code_challenge"
        ) as mock_code_challenge,
        ...
        mock_urandom.return_value = (
            b"\x00" * 32
        )  # Mock 32 bytes of zeros for state

        expected_state = (
            base64.urlsafe_b64encode(b"\x00" * 32).rstrip(b"=").decode("utf-8")
        )

        # Mock WebApplicationClient.prepare_request_uri
        with patch.object(
            WebApplicationClient, "prepare_request_uri"
        ) as mock_prepare_uri:
            mock_prepare_uri.return_value = (
...
def test_fitbit_callback_success_new_association(
    app, participant_auth_client, study_subject, fitbit_api
):
    ...
    query_params = {"state": "test_state", "code": "test_code"}

    with patch.object(
        WebApplicationClient, "prepare_token_request"
    ) as mock_prepare_token_request:
...
def test_fitbit_callback_state_mismatch(
    app, participant_auth_client, study_subject
):
    with participant_auth_client.session_transaction() as sess:
        sess["oauth_state"] = "correct_state"
        sess["oauth_code_verifier"] = "test_code_verifier"

    query_params = {"state": "incorrect_state", "code": "test_code"}
...
def test_fitbit_callback_missing_code(
    app, participant_auth_client, study_subject
):
    with participant_auth_client.session_transaction() as sess:
        sess["oauth_state"] = "test_state"
...
def test_fitbit_callback_token_exchange_failure(
    app, participant_auth_client, study_subject
):
    with participant_auth_client.session_transaction() as sess:
        sess["oauth_state"] = "test_state"
...
def test_fitbit_callback_missing_fitbit_api_entry(
    app, participant_auth_client, study_subject
):
    with participant_auth_client.session_transaction() as sess:
        sess["oauth_state"] = "test_state"
...
def test_fitbit_callback_error_storing_tokens(
    app, participant_auth_client, study_subject, fitbit_api
):
    with participant_auth_client.session_transaction() as sess:
        sess["oauth_state"] = "test_state"
...
def test_fitbit_callback_token_scope_validation(
    app, participant_auth_client, study_subject, fitbit_api
):
    with participant_auth_client.session_transaction() as sess:
        sess["oauth_state"] = "test_state"
...
def test_fitbit_sleep_list(
    ...
    with patch(
        "backend.views.api.fitbit.get_fitbit_oauth_session"
    ) as mock_get_session:
...
def test_fitbit_sleep_list_not_linked(
    app, participant_auth_client, study_subject, fitbit_api
):
