...
def aws_account_provider():
    ...
    provider.get_aws_access_key_id = MagicMock(return_value=mock_aws_access_key_id)
    provider.get_aws_secret_access_key = MagicMock(return_value=mock_aws_secret_access_key)
    provider.client = MagicMock()
    provider.client.meta.region_name = mock_aws_region
    provider.client.get_caller_identity = MagicMock(return_value={"Account": mock_account_id})
