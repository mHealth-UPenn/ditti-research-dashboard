...
from tests.tests_install.tests_local_providers.mock_docker_provider import (
    docker_provider,
)
...
def test_create_network_api_error(docker_provider_mock: DockerProvider):
    ...
    docker_provider_mock.docker_client.networks.create.side_effect = (
...
def test_run_postgres_container_success(docker_provider_mock: DockerProvider):
    ...
    mock_container.exec_run.return_value = MagicMock(
        exit_code=0, output=b"accepting connections"
    )
    ...
    assert (
        call_args["name"] == docker_provider_mock.config.postgres_container_name
    )
...
def test_run_postgres_container_error(docker_provider_mock: DockerProvider):
    ...
    docker_provider_mock.docker_client.containers.run.side_effect = (
        docker.errors.ContainerError(
            "Container Error", "exit_status", "command", "image", "stderr"
        )
    )
...
def test_build_wearable_data_retrieval_container_success(
    docker_provider_mock: DockerProvider, shutil_mock: MagicMock
):
    ...
    assert (
        call_args["tag"]
        == docker_provider_mock.config.wearable_data_retrieval_container_name
    )
...
def test_build_wearable_data_retrieval_copy_error(
    docker_provider_mock: DockerProvider, shutil_mock: MagicMock
):
...
def test_build_wearable_data_retrieval_build_error(
    docker_provider_mock: DockerProvider, shutil_mock: MagicMock
):  # Mock shutil to avoid copying shared files
    """Test handling of unexpected error when building wearable data retrieval container."""
    docker_provider_mock.docker_client.images.build.side_effect = (
        docker.errors.BuildError("Build Error", "stderr")
    )
...
def test_build_wearable_data_retrieval_unexpected_error(
    docker_provider_mock: DockerProvider, shutil_mock: MagicMock
):  # Mock shutil to avoid copying shared files
    """Test handling of unexpected error when building wearable data retrieval container."""
    docker_provider_mock.docker_client.images.build.side_effect = Exception(
        "Unexpected Error"
    )
...
def test_run_wearable_data_retrieval_container_success(
    docker_provider_mock: DockerProvider,
):
    ...
    assert (
        ...
        == docker_provider_mock.env_file_provider.get_wearable_data_retrieval_env()
    )
    assert call_args["detach"] is True


def test_run_wearable_data_retrieval_container_error(
    docker_provider_mock: DockerProvider,
):
...
def test_run_wearable_data_retrieval_unexpected_error(
    docker_provider_mock: DockerProvider,
):
    """Test handling of unexpected error when running wearable data retrieval container."""
    docker_provider_mock.docker_client.containers.run.side_effect = Exception(
        "Unexpected Error"
    )
...
def test_get_container_success(docker_provider_mock: DockerProvider):
    ...
    assert (
        result == docker_provider_mock.docker_client.containers.get.return_value
    )
    docker_provider_mock.docker_client.containers.get.assert_called_once_with(
        "test-container"
    )
...
def test_get_container_not_found(docker_provider_mock: DockerProvider):
    ...
    docker_provider_mock.docker_client.containers.get.side_effect = (
...
def test_get_network_not_found(docker_provider_mock: DockerProvider):
    ...
    docker_provider_mock.docker_client.networks.get.side_effect = (
...
def test_uninstall_success(docker_provider_mock: DockerProvider):
    ...
    docker_provider_mock.get_container = MagicMock(
        side_effect=[mock_postgres_container, mock_wearable_container]
    )
    ...
    docker_provider_mock.get_container.assert_any_call(
        docker_provider_mock.config.postgres_container_name
    )
    docker_provider_mock.get_container.assert_any_call(
        docker_provider_mock.config.wearable_data_retrieval_container_name
...
def test_uninstall_not_found(docker_provider_mock: DockerProvider):
    ...
    docker_provider_mock.get_container = MagicMock(
        side_effect=[
            DockerSDKError("Container not found"),
            DockerSDKError("Container not found"),
        ]
    )
    docker_provider_mock.get_network = MagicMock(
        side_effect=DockerSDKError("Network not found")
    )
