...
class TestParticipantAuthController:
    ...
    def test_get_or_create_user_existing(self, mock_db, mock_study_subject, app, auth_controller):
        ...
        mock_study_subject.query.filter_by.return_value.first.return_value = mock_existing_subject
        mock_token = {"id_token": "test-token"}
        mock_userinfo = {"sub": "ditti_id_123"}

        with app.app_context():
            # Override the actual method call
            with patch.object(auth_controller, "get_or_create_user") as mock_get_or_create:
